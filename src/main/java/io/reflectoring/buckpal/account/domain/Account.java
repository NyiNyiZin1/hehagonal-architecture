 ...
 * only contains a window of the latest account activities. The total balance of
 * the account is the sum of a baseline balance that was valid before the first
...
public class Account {
	 ...
	 * before the first activity in the activityWindow.
	 */
	@Getter
	...
	public Money calculateBalance() {
		System.err.println("baselineBalance>"+this.baselineBalance+">>"+"calculateBalance>"+this.activityWindow.calculateBalance(this.id));
		return Money.add(this.baselineBalance, this.activityWindow.calculateBalance(this.id));
	}

	/**
	 * Tries to withdraw a certain amount of money from this account. If successful,
	 * creates a new activity with a negative value.
	 * 
	...
	private boolean mayWithdraw(Money money) {
		System.err.println("calculateBalance>"+this.calculateBalance()+">>>"+"api send money>"+money.negate());
		//calculateBalance is database source account id balance
		//money.negate() is api send balance
		return Money.add(this.calculateBalance(), money.negate()).isPositiveOrZero();
	}

	/**
	 * Tries to deposit a certain amount of money to this account. If sucessful,
